import sys, socket, threading, pickle, ssl
...
class OP:
  ...
  def getRouters(self):
    ...
    ssocket = self.connectTLSSocket(DIR_SERVER, DIR_PORT)
    ssocket.send(b'G')
  ...
  def createTLSSocket(self, server, port):
   ...
   ssocket.listen()
   TLSsock = context.wrap_socket(ssocket, server_hostname=server,do_handshake_on_connect=False)
   TLSsock.do_handshake()  
   return TLSsock

  def connectTLSSocket(self, server, port):
   context = ssl.SSLContext(ssl.PROTOCOL_TLSv1_2)
   context.load_verify_locations(capath=CADIR)
   context.verify_mode = ssl.CERT_REQUIRED
   context.check_hostname =  True
  ...
  def createCircuit(self, routers):
    ...
    ssocket = self.connectTLSSocket(server, int(port))
    # calc gx1
    gx1 = pow(g, x1, p)
    ...
    if cmd == b'\x02':
      ...
      nonce = get_random_bytes(8)
      nonces.append(sha256(nonce))
      data = nonce + self.padData(21, addr) + cipherText
